...
export default class StatusCard extends Component {
  ...
  getLineGraphData() {
    if(this.props.status.data.length > 0){
      if(this.props.status.data.length > 30){
        var reverse=this.props.status.data.slice(0).reverse();
        reverse.length = 30;
        var complete = reverse.slice(0).reverse();
      } else {
        var complete = this.props.status.data
      }
      var graphData = complete.map((i, j) => {
        return { "x":j, "y":i }
      })
    } else {
